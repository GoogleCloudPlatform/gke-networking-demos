...
gcloud deployment-manager deployments create static-ip-deployment1 \
	--template static-ip.jinja --project "$1" \
...
gcloud deployment-manager deployments create static-ip-deployment1 \
	--template static-ip.jinja --project "$2" \
...
VPN1_IP=$( gcloud compute addresses list --project "$1" | grep vpn1 \
	| awk '{ print $3 }' )
VPN2_IP=$( gcloud compute addresses list --project "$1" | grep vpn2 \
	| awk '{ print $3 }' )
VPN3_IP=$( gcloud compute addresses list --project "$2" | grep vpn3 \
	| awk '{ print $3 }' )
VPN4_IP=$( gcloud compute addresses list --project "$2" | grep vpn4 \
	| awk '{ print $3 }' )

### Create VPN connection for network1 and network2 in us-east1 &
### us-west1 regions
gcloud deployment-manager deployments create vpn1-deployment \
	--template vpn-custom-subnet.jinja --project "$1" \
	--properties "region:us-west1,network:projects/$1/global/networks/network1,\
...
gcloud deployment-manager deployments create vpn2-deployment \
	--template vpn-custom-subnet.jinja --project "$1" \
	--properties "region:us-east1,network:projects/$1/global/networks/network1,\
...
gcloud deployment-manager deployments create vpn3-deployment \
	--template vpn-custom-subnet.jinja --project "$2" \
	--properties "region:us-west1,network:projects/$2/global/networks/network2,\
...
gcloud deployment-manager deployments create vpn4-deployment \
	--template vpn-custom-subnet.jinja --project "$2" \
	--properties "region:us-east1,network:projects/$2/global/networks/network2,\
